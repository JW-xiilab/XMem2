...
class VideoReader(Dataset):
    ...
    """
    def __init__(self, vid_name, video_path, mask, out_dir, size=-1, to_save=None, use_all_masks=False, size_dir=None):
        """
        ...
        self.reference_mask = self.mask  #Image.open(path.join(mask_dir, sorted(os.listdir(mask_dir))[0])).convert('P')
        ...
        if path.isfile(self.video_path):
            ...
            self.image_dir = path.join(out_dir, self.vid_name, 'images') #self.tmp_dir.name
