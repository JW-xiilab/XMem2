...
sys.path.append('/DATA_17/kjw/01-SM_project/XMem2')
...
class BaseSegmenter:
    def __init__(self, SAM_checkpoint, model_type, device=None):
        ...
        if not device:
            ...
            self.device = 'cpu'
        print(f"Initializing BaseSegmenter to {self.device}")
        self.torch_dtype = torch.float16 if 'cuda' in self.device.type else torch.float32
    ...
    def predict(self, prompts, mode, multimask=True):
        ...
        assert mode in ['point', 'mask', 'box', 'both'], 'mode must be point, mask, or both'
        ...
        if mode == 'point':
            ...
            masks, scores, logits = self.predictor.predict(box=prompts['box_coords'],
                                multimask_output=multimask)
        elif mode == 'mask':
